...
function module.set()
    for _, o in pairs(module.native_objects) do
     ...
     -- line textobject
     -- todo cleanup
     vim.keymap.set({'x', 'o'}, 'ap', ':<c-u>normal! $v0<cr>', {noremap=true, silent=true, desc="Line"})
     vim.keymap.set({'x', 'o'}, 'ip', ':<c-u>normal! $v^<cr>', {noremap=true, silent=true, desc="Line"})
     vim.keymap.set({'x', 'o'}, 'p' , ':<c-u>normal! V<cr>',   {noremap=true, silent=true, desc="Line"})

    vim.keymap.set({'x', 'o', 'n'}, 'm', '<plug>(matchup-%)', {remap=true})
    vim.keymap.set({'x', 'o', 'n'}, 'gm', '<plug>(matchup-g%)', {remap=true})
    vim.keymap.set({'n', 'x', 'o'}, ']m', '<plug>(matchup-]%)', {remap=true})
    vim.keymap.set({'n', 'x', 'o'}, '[m', '<plug>(matchup-[%)', {remap=true})
    -- vim.keymap.set({'n', 'x', 'o'}, 'zm', '<plug>(matchup-z%)', {remap=true})
    vim.keymap.set({'x', 'o'}, 'im', '<plug>(matchup-i%)', {remap=true})
    vim.keymap.set({'x', 'o'}, 'ym', '<plug>(matchup-a%)', {remap=true})
    vim.keymap.set({'x', 'o'}, 'om', '<plug>(matchup-s%)', {remap=true})
    vim.keymap.set({'n'}, 'xom', '<plug>(matchup-ds%)', {remap=true})
    vim.keymap.set({'n'}, 'dom', '<plug>(matchup-ds%)', {remap=true})
    vim.keymap.set({'n'}, 'rom', '<plug>(matchup-cs%)', {remap=true})
    ...
    for k, v in pairs(ai_objects) do
        ...
        --     return MiniAi.move_cursor("left"/"right", "a"/"i", "f", {search_method=""})
        

        -- define shortcut jump like tf (to function)
        no_shortcut = {[to.next]=true, [to.last]=true, [to.inside]=true, [to.around]=true}
        if not no_shortcut[k] then
            vim.keymap.set({"n", "x", "o"}, to.go_to .. k, function() 
                return MiniAi.move_cursor("left", "a", k, {search_method="next"})
            end, {silent=true})
            vim.keymap.set({"n", "x", "o"}, to.go_to_end .. k, function() 
                return MiniAi.move_cursor("right", "a", k, {search_method="cover_or_next"})
            end, {silent=true})
            vim.keymap.set({"n", "x", "o"}, to.go_to .. to.inside .. k, function() 
                return MiniAi.move_cursor("left", "i", k, {search_method="next"})
            end, {silent=true})
            vim.keymap.set({"n", "x", "o"}, to.go_to_end .. to.inside .. k, function() 
                return MiniAi.move_cursor("right", "i", k, {search_method="cover_or_next"})
            end, {silent=true})    
        end

        -- define next jump like tnf (to next function)
        vim.keymap.set({"n", "x", "o"}, to.go_to .. to.next .. k, function() 
            return MiniAi.move_cursor("left", "a", k, {search_method="next"})
        end, {silent=true})
        vim.keymap.set({"n", "x", "o"}, to.go_to_end .. to.next .. k, function() 
            return MiniAi.move_cursor("right", "a", k, {search_method="cover_or_next"})
        end, {silent=true})
        vim.keymap.set({"n", "x", "o"}, to.go_to .. to.inside .. to.next .. k, function() 
            return MiniAi.move_cursor("left", "i", k, {search_method="next"})
        end, {silent=true})
        vim.keymap.set({"n", "x", "o"}, to.go_to_end .. to.inside .. to.next .. k, function() 
            return MiniAi.move_cursor("right", "i", k, {search_method="cover_or_next"})
        end, {silent=true})

        -- define last jump like tlf (to last function)
        vim.keymap.set({"n", "x", "o"}, to.go_to .. to.last .. k, function() 
            return MiniAi.move_cursor("left", "a", k, {search_method="cover_or_prev"})
        end, {silent=true})
        vim.keymap.set({"n", "x", "o"}, to.go_to_end .. to.last .. k, function() 
            return MiniAi.move_cursor("right", "a", k, {search_method="prev"})
        end, {silent=true})
        vim.keymap.set({"n", "x", "o"}, to.go_to .. to.inside .. to.last .. k, function() 
            return MiniAi.move_cursor("left", "i", k, {search_method="cover_or_prev"})
        end, {silent=true})
        vim.keymap.set({"n", "x", "o"}, to.go_to_end .. to.inside .. to.last .. k, function() 
